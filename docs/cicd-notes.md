...
## 11. 生产事故：一次文档改动把整机压死（2026-09）

**现象**：push 一个只改 README 的 commit → 触发 `deploy` job → 服务器上开跑全量
`docker build`；随后整台机器（连 SSH 登录）都卡死，站点无法访问。

**根因链**（不是单点故障，是「触发条件过粗 + 资源无上限」叠加）：

1. `on: push: branches: [main]` 对文件类型一视同仁 —— 改文档也要重新构建镜像；
2. 镜像在**同一台机器**上构建，与 PG / Redis / MinIO / app 争内存与磁盘；
3. 构建缓存无上限：`docker image prune -f` 只清 dangling，按 git sha 打 tag 的历史镜像**全留着**；
4. `docker-compose.prod.yaml` 没有日志轮转 —— 容器 json-file 日志无限增长；
5. 磁盘写满后 PG 写不进 WAL、docker 写不了层 → 全机（含 sshd）无响应，只能强制重启。

**处置**：Actions 里取消该 run（`Upload source` 取消、`Build & deploy` 直接 skipped，
服务器侧一次构建都没跑）→ 控制台强制重启实例（容器 `restart: unless-stopped` 自动拉起）。
