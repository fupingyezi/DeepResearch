...
3. **放置生产环境变量文件 `.env.production`**：
   ...
   - `MODEL_KEY_ENC_SECRET`：`openssl rand -base64 32`
...
chmod +x scripts/*.sh
bash scripts/deploy-remote.sh app-image.tar.gz mini-deepresearch:<git_sha>
```
...
- 镜像 tag 使用 git 短 sha（前 12 位）：`mini-deepresearch:<git_sha>`。
