 ...
 *
 * 在一轮回答的「最终 parts[]」上：
 *  1) 抽取 `<task_summary>...</task_summary>` 块为独立 task_summary part。
 ...
 *     （应对模型把 task_summary 误写在报告内部的常见情形）
...
const TASK_SUMMARY_TAG_OPEN = /<task_summary>([\s\S]*)$/;
...
export function extractFinalMessageParts(
  ...
  for (let i = 0; i < parts.length; i++) {
    if (isScannable(parts[i])) scanIndices.push(i);
  }
  ...
  // 1) 抽 task_summary（优先于 artifact 提升，避免被并入报告正文）
  if (!hasTaskSummary) {
    const summaryHit = pickTaskSummary(working, scanIndices);
    if (summaryHit) {
      working = summaryHit.workingAfterStrip;
      taskSummaryPart = makeTaskSummaryPart(summaryHit.text);
    }
  }

  // 2) H1 + ≥2 个 H2 启发式 → 整段 text/reasoning 提升为 artifact
  if (!hasArtifact) {
    const reportHit = pickReportFromScannables(working, scanIndices);
    if (reportHit) {
      working = reportHit.workingAfterStrip;
      artifactPart = makeArtifactPart(reportHit.markdown, fallbackTitle);
    }
  }

  // 3) 兜底 a：若 task_summary 仍未抽到，但 artifact 抽出了，尝试在 artifact 内再扫一次
  //    （应对模型把 <task_summary> 误写在报告内部的情形）
  if (!taskSummaryPart && !hasTaskSummary && artifactPart) {
    const innerMatch =
      artifactPart.content.markdown.match(TASK_SUMMARY_TAG) ||
      artifactPart.content.markdown.match(TASK_SUMMARY_TAG_OPEN);
    if (innerMatch) {
      const text = innerMatch[1].trim();
      if (text.length > 0) {
        taskSummaryPart = makeTaskSummaryPart(text);
        const stripRe =
          innerMatch === artifactPart.content.markdown.match(TASK_SUMMARY_TAG)
            ? TASK_SUMMARY_TAG
            : TASK_SUMMARY_TAG_OPEN;
        artifactPart = {
          ...artifactPart,
          content: {
            ...artifactPart.content,
            markdown: artifactPart.content.markdown.replace(stripRe, '').trim(),
          },
        };
      }
    }
...
/**
 * 从扫描范围内挑选 task_summary：先匹配闭合标签，再兜底未闭合（仅当紧邻末尾）。
 * 命中后把对应 part 的对应片段 strip 掉。
 */
function pickTaskSummary(
  parts: MessagePart[],
  scanIndices: number[],
): { text: string; workingAfterStrip: MessagePart[] } | null {
  for (const i of scanIndices) {
    const part = parts[i];
    if (!part || !isScannable(part)) continue;
    const closed = part.content.text.match(TASK_SUMMARY_TAG);
    if (closed) {
      ...
      next[i] = {
        ...part,
        content: { text: part.content.text.replace(TASK_SUMMARY_TAG, '').trim() },
      };
      return { text, workingAfterStrip: next };
    }
  }

  // 未闭合兜底：仅在最后一个扫描 part 中匹配 `<task_summary>` 到 EOF
  for (let k = scanIndices.length - 1; k >= 0; k--) {
    const i = scanIndices[k];
    const part = parts[i];
    if (!part || !isScannable(part)) continue;
    const open = part.content.text.match(TASK_SUMMARY_TAG_OPEN);
    if (!open) continue;
    ...
    next[i] = {
      ...part,
      content: { text: part.content.text.replace(TASK_SUMMARY_TAG_OPEN, '').trim() },
    };
    return { text, workingAfterStrip: next };
  }
  return null;
...
/**
 * 从扫描范围内挑选研究报告：定位首个含 `# {title}` 的 part，从首个 H1 处截到该 part 末尾。
 * 必须满足「截取段内 ≥2 个 H2 章节」才提升为 artifact，避免误把普通带标题的回答当报告。
 */
function pickReportFromScannables(
  parts: MessagePart[],
  scanIndices: number[],
): { markdown: string; workingAfterStrip: MessagePart[] } | null {
  for (const i of scanIndices) {
    const part = parts[i];
    if (!part || !isScannable(part)) continue;
    const text = part.content.text;
    const h1Match = text.match(/^#\s+.+$/m);
    if (!h1Match || typeof h1Match.index !== 'number') continue;

    const candidate = text.slice(h1Match.index).trim();
    const h2Count = (candidate.match(/^##\s+/gm) ?? []).length;
    if (h2Count < 2) continue;

    const next = [...parts];
    const before = text.slice(0, h1Match.index).trim();
    next[i] = { ...part, content: { text: before } };
    return { markdown: candidate, workingAfterStrip: next };
  }
  return null;
}

/** 从报告正文首个 H1 标题推断 title，缺省回退到 fallbackTitle */
function makeArtifactPart(reportContent: string, fallbackTitle: string): ArtifactPart {
  let title = '';
  const headingMatch = reportContent.match(/^#\s+(.+?)\s*$/m);
 ...
 * `result` 首句、再缺失则用「（无产出）」），按 deer-flow _summarize_completed_tasks
