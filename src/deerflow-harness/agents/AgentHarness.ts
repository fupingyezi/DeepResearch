...
import { StreamProcessor } from "@/agents/modules/StreamProcessor";
import { EventStreamAdapter } from "@/agents/eventStream/EventStreamAdapter";
import { AgentEventEmitter } from "@/agents/modules/AgentEventEmitter";
import {
  HarnessConfig,
  HarnessContext,
  HarnessLifecycle,
...
import { HooksManager } from "../middleware/HooksManager";
...
export class AgentHarness {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      // 创建 Agent 实例
      // 注意：不要预先调用 model.bindTools()，因为 createAgent（ReactAgent）
      // 内部会自动绑定工具。如果传入已绑定工具的模型，会抛出 MultipleToolsBoundError。
